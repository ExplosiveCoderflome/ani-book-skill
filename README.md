...
如果你明确表示“我没有想法”，它会先给你五条风格不同的一句话开书种子：强钩子、人物成长、设定奇观、关系牵引和悬念追查。你选中或改写其中一条、或者给出自己的灵感后，Codex 会生成两份在冲突、主角路径、推进循环和调性上明显不同的新书简报预览。选择、混合或委托一份方向后，才确认频道、发布形态和主要阅读回报，并固化为 `novel-brief.md`。
...
本次更新：开书入口增加“五条灵感 → 双方向简报”流程，并保留 Skill 镜像新鲜度检查与 `0.3.0` 版本标记。

- 明确没有想法时，先获得五条风格不同的一句话开书种子；选中、改写或自填后，再比较两份明显不同的新书简报预览。
- 只有选择、混合或委托其中一份方向后，才确认开书设置并固化权威简报；预览不会提前驱动世界、卷或章节生产。

- 每次首次使用 Skill、以及变更 Skill 表面后，都会先比对权威源与已安装镜像；发现缺失或差异时会明确报告，避免在过期副本上继续生产。
- 验证完成后可使用只增不删的同步命令更新镜像，再次检查确认一致；不会静默替换缺失的权威源。
